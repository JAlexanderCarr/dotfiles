...
if [[ "$OS" == "darwin" ]]; then
    ...
    cd /tmp
    wget "https://download.java.net/java/GA/jdk${JAVA_VERSION%%.*}/$JAVA_VERSION/GPL/openjdk-$JAVA_VERSION_linux-$ARCH.tar.gz"
    tar -xzf openjdk-$JAVA_VERSION_linux-$ARCH.tar.gz
    mkdir -p /usr/lib/jvm
